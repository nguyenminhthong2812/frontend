...
export class PhimService {
  ...
  LayDanhSachPhim():Observable<any>{
    let api = `http://svcy2.myclass.vn/api/QuanLyPhim/LayDanhSachPhim?MaNhom=GP03`;
    let Observe = this._http.get(api);
    return Observe;
  }
  LayChiTietPhim(maPhim):Observable<any>{
    let api = `http://svcy2.myclass.vn/api/QuanLyPhim/LayChiTietPhim?MaPhim=${maPhim}`;
    let Observe = this._http.get(api);
    return Observe;
  }
  ThemPhim(phim):Observable<any>{
    let api = `http://svcy2.myclass.vn/api/QuanLyPhim/ThemPhimMoi`;
  ...
  SuaPhim(phim):Observable<any>{
    let api = `http://svcy2.myclass.vn/api/QuanLyPhim/CapNhatPhim`;
    let header = new HttpHeaders();
    header.append('Content-Type','application/json;charset=UTF-8');
    let Observe = this._http.post(api,phim,{headers:header});
    return Observe;
  ...
  XoaPhim(maphim):Observable<any>{
    let api = `http://svcy2.myclass.vn/api/QuanLyPhim/XoaPhim?MaPhim=${maphim}`;
